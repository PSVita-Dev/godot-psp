...
def configure(env):
    ...
    wut_path = devkitpro_path + "/libogc"
    ...
    arch = ['-mrvl', '-mcpu=750', '-meabi', '-mhard-float', '-DHW_RVL', '-fdata-sections']
    env.Append(CCFLAGS=['-g','-MMD', '-MP' ,'-Wall', '-std=gnu++11'] + arch)
    env.Append(CCFLAGS=[ '-DGEKKO', '-D__WII__','-DNEED_LONG_INT', '-DLIBC_FILEIO_ENABLED','-DNO_SAFE_CAST', '-DPTHREAD_NO_RENAME'])
    
    env.Append(CPPPATH=[wut_path+"/include"])
    env.Append(LIBPATH=[wut_path+"/lib", wut_path+"/lib/wii", devkitpro_path+"/portlibs/ppc/lib/"])
    ...
    env.Append(LINKFLAGS=arch+['-Wl,-Map,all.map'])
    env.Append(LIBS=["opengx", "fat","wiiuse", "wiikeyboard","bte", "m", "ogc"])
